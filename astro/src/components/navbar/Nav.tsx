...
function TopNav() {
  const [isScrollTop, setIsScrollTop] = createSignal(true)
  createEffect(() => {
    const onScroll = () => {
      console.log(window.scrollY === 0)
      setIsScrollTop(window.scrollY === 0)
      setStyles(isScrollTop() ? "-translate-y-24" : "translate-y-0")
    }
  ...
  const [styles, setStyles] = createSignal(
    isScrollTop() ? "-translate-y-24" : "translate-y-0",
  )

  return (
    <nav
      class={`${styles()} fixed left-0 top-0 z-40 flex w-[100vw] justify-between bg-neutral-900/20 pr-2 text-center backdrop-blur-xl transition duration-500 ease-in-out`}
    >
